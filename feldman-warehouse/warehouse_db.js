    ...
    try {
        ...
        if (itemsCount === 0) {
            const sampleItems = [
                ...
                { name: 'מיסב כדורי 6308 2RS', category: 'מיסבים', shelf: '1-2', qty: 2, minQty: 4 },
                ...
                { name: 'גלגל שיניים חלזוני 1:30', category: 'גלגלי שיניים', shelf: '3-3', qty: 1, minQty: 2 },
                { name: 'מנוע חשמלי 1.5HP 1400RPM', category: 'מנועים', shelf: '5-1', qty: 3, minQty: 1 },
                { name: 'מנוע תלת פאזי 5.5KW', category: 'מנועים', shelf: '5-2', qty: 0, minQty: 1 },
            ...
            for (const item of sampleItems) {
                await client.execute({
                    sql: "INSERT INTO inventory_items (name, category, shelf_location, quantity, min_quantity) VALUES (?, ?, ?, ?, ?)",
                    args: [item.name, item.category, item.shelf, item.qty, item.minQty]
                });
            }
